...
class SportContainer(val summary: SportSummary, val sportDetail: SportDetail) {
    ...
    private val timeList: List<Int>
        get() = sportDetail.time?.split(';')?.filter { it.isNotEmpty() }?.map { it.toInt() }
            ?: listOf()

    private val latitudeList: List<Long>
        get() = sportDetail.longitude_latitude?.split(';')?.filter { it.isNotEmpty() }
            ?.map { it.split(',')[0].toLong() } ?: listOf()

    private val longitudeList: List<Long>
        get() = sportDetail.longitude_latitude?.split(';')?.filter { it.isNotEmpty() }
            ?.map { it.split(',')[1].toLong() } ?: listOf()

    private val altList: List<Long>
        get() {
            val noZeppValue = -2000000L
            val tmp: MutableList<Long> =
                sportDetail.altitude?.split(';')?.filter { it.isNotEmpty() }?.map { it.toLong() }
                    ?.toMutableList() ?: java.util.ArrayList()
    ...
    private fun timedSpeed(from: Long, to: Long): Map<Long, Float> {
        val elements = sportDetail.speed?.split(';')?.filter { it.isNotEmpty() }?.map {
            val (timeDelta, speed) = it.split(','); Pair(
            timeDelta.toLong(),
            speed.toFloat()
        )
        } ?: listOf()
    ...
    private fun timedGait(from: Long, to: Long): Map<Long, GaitContainer> {
        ...
        return timedStep.keys.associateWith {
            GaitContainer(
                timedStep[it]!!,
                timedStride[it]!!,
                timedFreq[it]!!
    ...
    private fun <T : Number> timedCumulative(
        from: Long,
        to: Long,
        elements: List<Pair<Long, T>>,
        initValue: T,
        operator: (T, T) -> T
    ): java.util.HashMap<Long, T> {
    ...
    private fun <T : Number> timedFixed(
        from: Long,
        to: Long,
        elements: List<Pair<Long, T>>,
        initialValue: T
